#!/bin/bash
# ensure-maison-yundera-mirror.sh - Project the `yundera` stack into Maison's layout.
#
# Writes:
...
# compose lives at /DATA/AppData/casaos/apps/yundera — two levels under Maison's
...
#
# This is the same promotion ensure-maison-app-mirror.sh performs for CasaOS apps,
...
# ensure-maison-app-mirror.sh explicitly skips `yundera` and must keep doing so.
#
...
#
# COPY, NOT HARDLINK — same reasoning as ensure-maison-app-mirror.sh: Maison's
...
#
# CONSEQUENCE OF PROMOTING, ACCEPTED: a managed tile is one Maison may `compose up`
...
# Maison's flat layout. What we must not do is clobber a compose we did not write:
...
chmod 600 "$TMP_ENV"
{
    ...
    echo "# Copy of $UNIFIED_ENV, written by scripts/self-check/ensure-maison-yundera-mirror.sh"
    echo "# so Maison renders this stack's compose exactly as the template does."
...
#
# HERMETIC (`env -i`) on both sides — the same reasoning as ensure-maison-app-mirror.sh:
...
log_info "Maison yundera mirror: $DST_DIR is in sync"
exit 0
