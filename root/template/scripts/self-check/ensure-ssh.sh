...
# 2. Package
# --------------------------------------------------------------------------
try "$YND_TEMPLATE/scripts/tools/ensure-packages.sh" openssh-server
...
if [ "$DPKG_SAFE" = "0" ]; then
    ...
    [ -x "$YND_TEMPLATE/scripts/tools/wait-for-apt-lock.sh" ] && \
        "$YND_TEMPLATE/scripts/tools/wait-for-apt-lock.sh"
