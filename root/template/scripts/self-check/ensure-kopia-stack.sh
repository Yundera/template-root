...
# — the repository, its caches and its logs all live in $KOPIA_ENGINE_DIR, which the
# stack only mounts — so taking them down destroys nothing and leaves Maison on the
# one-shot path it used
